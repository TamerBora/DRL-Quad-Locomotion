...
\begin{frame}{Three networks, and two value functions}
  ...
  \begin{tikzpicture}[x=1cm, y=1cm]
    ...
    \node[critic, text width=1.7cm] (qf) at (11.9,1.25) {$Q_f$};
    \node[critic, text width=1.7cm] (qg) at (11.9,-0.75){$Q_g$};
  ...
  \begin{block}{\small What the two heads mean}
    ...
    Same body, same input; only the output heads differ. \emph{Which} action is fed in ---
    and which head answers --- depends on the update (next slide).}
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      {\scriptsize The teacher's advantage is
       \textcolor{cGuide!70!black}{\textbf{capped by construction}} --- contradicting the premise
       that a fully-observed teacher is better.\par}
    \end{column}
    \begin{column}{0.47\textwidth}
      \begin{center}
      ...
      {\scriptsize Each actor optimizes against \emph{its own} fixed point, so the teacher can
       run ahead --- while the student still optimizes the \textbf{deployed} policy's value.\par}
    \end{column}
  \end{columns}
  \vspace{0.5em}
  \begin{center}
