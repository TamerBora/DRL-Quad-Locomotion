...
class TorchUniformBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        # Per-transition bootstrap discount γ^m, where m = steps from t to the
        # state we bootstrap from (m = n for windows with no boundary; m = k+1
        # if the episode ends — timeout/failure — at window-step k). Exact n-step
        # discount per eq 11 (term 3 uses γ^{k+1}, not a fixed γ^n).
    ...
    def _get_n_step_prev_transition(self) -> Batch:
        ...
        # Bootstrap discount: default γ^n (bootstrap from the end of the window).
        n_step_discount = torch.full_like(n_step_reward, float(self._gamma ** self._n_step))
        ...
        for n_step_idx in reversed(range(self._n_step - 1)):
            ...
            # assign next observation + bootstrap discount starting from done.
            # Iterating reversed means the EARLIEST done in the window wins, so
            # the bootstrap horizon is m = n_step_idx + 1 → discount γ^{k+1}.
            done_mask = done.bool()
            ...
            n_step_discount[done_mask] = float(self._gamma ** (n_step_idx + 1))
