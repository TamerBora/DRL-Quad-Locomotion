...
def _compute_categorical_td_target(
    ...
    gamma: torch.Tensor,  # (B,) per-sample bootstrap discount γ^m (exact n-step)
    num_bins: int,
    min_v: float,
    max_v: float,
) -> torch.Tensor:
...
def update_critic(
    ...
    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
        ...
        with torch.no_grad():
            ...
            # Compute target probs. The bootstrap discount is per-sample γ^m
            # from the buffer (exact n-step, eq 11): γ^n for full windows, γ^{k+1}
            # if the episode ends at window-step k. Replaces a fixed γ**n_step.
            target_probs = _compute_categorical_td_target(
                ...
                gamma=batch["discount"],  # type: ignore  # (B,) per-sample γ^m
                num_bins=num_bins,
                min_v=min_v,
                max_v=max_v,
            )
