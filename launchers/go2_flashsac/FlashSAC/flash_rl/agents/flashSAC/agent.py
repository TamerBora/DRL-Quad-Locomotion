...
class FlashSACConfig:
    ...
    # Actor-side action-rate smoothness coefficient β for
    # ``β · E[Σ_dim (π(s') − π(s))²]``. 0.0 disables the term.
    actor_smooth_beta: float = 0.0

    # RSL-RL-SAC controlled init: initial (state-independent) policy std σ₀.
    init_sigma: float = 0.15
    # Decoupled temperature learning rate (paper uses 2e-5, ~10× smaller than the
    # actor/critic LR). Constant LR (no schedule) for α.
    temp_learning_rate: float = 2e-5
...
def _init_flashsac_networks(
    ...
    # Initialize temperature. Decoupled, constant LR for α (RSL-RL-SAC uses a
    # much smaller temperature LR than actor/critic; a fixed LR avoids the
    # shared cosine schedule driving α-adaptation to ~0 late in training).
...
def _update_networks(
    ...
    if do_actor_update:
        ...
        actor_info = update_actor(
            ...
            smooth_beta=cfg.actor_smooth_beta,
            device=device,
            use_amp=cfg.use_amp,
