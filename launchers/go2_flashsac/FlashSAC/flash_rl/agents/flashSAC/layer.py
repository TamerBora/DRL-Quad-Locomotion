...
class NormalTanhPolicy(nn.Module):
    def __init__(
        ...
        super().__init__()
        # RSL-RL-SAC controlled init (eq 7-8): plain Linear heads (NOT UnitLinear)
        # so they are excluded from the weight-normalisation pass and the init
        # persists. Mean head starts ~0 → initial action ≈ 0 → default joint
        # pose; log-std head starts state-independent at sigma = init_sigma.
        ...
        # Constant raw-bias that maps to log(init_sigma) through the smooth bound
        # below, so std ≡ init_sigma at init regardless of state.
        target_log_std = math.log(init_sigma)
        frac = (target_log_std - log_std_min) / (log_std_max - log_std_min)
        frac = min(max(frac, 1e-4), 1.0 - 1e-4)
        raw_bias = math.atanh(2.0 * frac - 1.0)
        nn.init.constant_(self.std_head.bias, raw_bias)
