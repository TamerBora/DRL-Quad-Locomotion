"""Render an MP4 of a FlashSAC or RSL-RL PPO checkpoint (Go2) headlessly.

Spins up an Xvfb virtual X server, runs the appropriate play.py against
it, captures the display with ffmpeg, then cleans everything up.

FlashSAC:
    python create_video.py <ckpt_dir>             # actor.pt + friends inside
RSL-RL PPO:
    python create_video.py <ckpt.pt> --rsl_rl     # single .pt model file

Outputs land in <ckpt or ckpt-parent>/videos/playback.mp4.
"""
...
_RSL_RL_PLAY_SCRIPT = Path(
    os.environ.get("ROBOT_LAB_DIR", os.path.expanduser("~/robotics/robot_lab"))
...
def main() -> int:
    ...
    parser.add_argument("--rsl_rl", action="store_true", default=False,
                        help="Render an RSL-RL PPO checkpoint (single .pt file) "
                             "using robot_lab's play.py instead of FlashSAC's.")
    parser.add_argument("--task", type=str,
                        default="RobotLab-Isaac-Velocity-Rough-Unitree-Go2-v0",
                        help="IsaacLab task ID, passed through to play.py.")
    args = parser.parse_args()

    _check_deps()
    play_script = _RSL_RL_PLAY_SCRIPT if args.rsl_rl else _PLAY_SCRIPT
    if not play_script.exists():
        sys.exit(f"play.py not found: {play_script}")

    ckpt = Path(args.checkpoint).expanduser().resolve()
    if args.rsl_rl:
        if not ckpt.is_file() or ckpt.suffix != ".pt":
            sys.exit(f"--rsl_rl expects a .pt model file, got: {ckpt}")
        videos_dir = ckpt.parent / "videos"
        # Tag the filename with the model name so multiple checkpoints don't
        # overwrite each other (e.g. model_4700 → playback_model_4700.mp4).
        if args.output_name == "playback.mp4":
            args.output_name = f"playback_{ckpt.stem}.mp4"
    else:
        if not ckpt.is_dir():
    ...
    # FlashSAC play.py accepts --num_episodes and --camera_switch_interval;
    # robot_lab's play.py doesn't, so we omit them in RSL-RL mode and let
    # the play loop run until ffmpeg finishes and we SIGKILL it below.
    if args.rsl_rl:
        play_cmd = [
            sys.executable, "-u", str(play_script),
            "--task", args.task,
            "--checkpoint", str(ckpt),
